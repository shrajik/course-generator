...
class DocumentService:
    ...
    # --- loading ----------------------------------------------------------
    def load(self, document_id: str) -> CourseDocument:
    ...
    async def export_pdf(self, document_id: str) -> tuple[Path, CourseDocument]:
        ...
        record = self.storage.load_course(document.course_id)
        record.pdf_path = str(path.relative_to(self.storage.course_dir(document.course_id)))
